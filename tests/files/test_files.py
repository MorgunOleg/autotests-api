...
class TestFiles:
    ...
    def test_create_file(self, files_client: FilesClient):
        ...
        # Дополнительная проверка на фактическое создание файла по ожидаемому пути (при необходимости)
    ...
    def test_get_file_with_incorrect_file_id(self, files_client: FilesClient):
        # 1. Отправление запроса на получение файла
        response = files_client.get_file_api(file_id="incorrect-file-id")
        response_data = ValidationErrorResponseSchema.model_validate_json(response.text)

        # 2. Проверяем, что код ответа соответствует ожиданиям (422 - Unprocessable Entity)
        assert_status_code(response.status_code, HTTPStatus.UNPROCESSABLE_ENTITY)

        # 3. Проверяем, что ответ API соответствует ожидаемой валидационной ошибке
        assert_get_file_with_incorrect_file_id_response(response_data)

        # 4. Дополнительно проверяем, что валидационная ошибка соответствует JSON-схеме
        validate_json_schema(response.json(), response_data.model_json_schema())
