...
@allure.step("Check file is accessible via URL")
def assert_file_is_accessible(url: str):
    ...
    """
    logger.info("Check file is accessible via URL")
    ...
    expected = ValidationErrorResponseSchema(
        details=[  # type: ignore
            ValidationErrorSchema(
                ...
                context={"min_length": 1},  # type: ignore # Минимальная длина строки должна быть 1 символ.
                message="String should have at least 1 character",  # type: ignore # Сообщение об ошибке.
                location=["body", "filename"]  # type: ignore # Ошибка возникает в теле запроса, поле "filename".
            )  # type: ignore
        ]
    )  # type: ignore
    ...
    expected = ValidationErrorResponseSchema(
        details=[  # type: ignore
            ValidationErrorSchema(
                ...
                context={"min_length": 1},  # type: ignore # Минимальная длина строки должна быть 1 символ.
                message="String should have at least 1 character",  # type: ignore # Сообщение об ошибке.
                location=["body", "directory"]  # type: ignore # Ошибка возникает в теле запроса, поле "directory".
            )  # type: ignore
        ]
    )  # type: ignore
    assert_validation_error_response(actual, expected)
...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что ответ на запрос данных файла по некорректному ID соответствует ожидаемой валидационной ошибке.
    ...
    expected = ValidationErrorResponseSchema(
        details=[  # type: ignore
            ValidationErrorSchema(
                type="uuid_parsing",  # Тип ошибки, связанной с неверным форматом ID файла.
                location=["path", "file_id"],  # type: ignore # Ошибка возникает в параметрах пути, поле "file_id".
                message=(  # type: ignore
                    "Input should be a valid UUID, invalid character: expected an optional prefix of `urn:uuid:` "
                    "followed by [0-9a-fA-F-], found `i` at 1"
                ),  # Сообщение об ошибке.
                input="incorrect-file-id",  # ID файла в некорректном формате.
                context={  # type: ignore
                    "error": (
                        "invalid character: expected an optional prefix of `urn:uuid:` "
                        "followed by [0-9a-fA-F-], found `i` at 1"
                    )
                },  # Ожидаемый валидный формат UUID.
            )  # type: ignore
        ]
    )  # type: ignore
    assert_validation_error_response(actual, expected)
